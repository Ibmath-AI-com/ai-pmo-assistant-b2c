...
class TemplateService:
    ...
    async def create(self, db: AsyncSession, data: dict, created_by: uuid.UUID, organization_id: uuid.UUID) -> Template:
        template = Template(
    ...
    async def get(self, db: AsyncSession, template_id: uuid.UUID, organization_id: uuid.UUID) -> Template:
        stmt = (
            select(Template)
            .options(*self._eager())
    ...
    async def update(self, db: AsyncSession, template_id: uuid.UUID, data: dict, updated_by: uuid.UUID, organization_id: uuid.UUID) -> Template:
    ...
    async def list_versions(self, db: AsyncSession, template_id: uuid.UUID, organization_id: uuid.UUID) -> list[TemplateVersion]:
        await self.get(db, template_id, organization_id)
        stmt = (
    ...
    async def get_version(self, db: AsyncSession, template_id: uuid.UUID, version_id: uuid.UUID, organization_id: uuid.UUID) -> TemplateVersion:
        await self.get(db, template_id, organization_id)
    ...
    async def save_version(self, db: AsyncSession, template_id: uuid.UUID, data: dict, created_by: uuid.UUID, organization_id: uuid.UUID) -> TemplateVersion:
        template = await self.get(db, template_id, organization_id)
    ...
    async def restore_version(self, db: AsyncSession, template_id: uuid.UUID, version_id: uuid.UUID, updated_by: uuid.UUID, organization_id: uuid.UUID) -> Template:
        template = await self.get(db, template_id, organization_id)
        version = await self.get_version(db, template_id, version_id, organization_id)
        await self._snapshot_version(db, template, updated_by)
    ...
    async def set_file_mappings(self, db: AsyncSession, template_id: uuid.UUID, mappings: list[dict], organization_id: uuid.UUID) -> list[TemplateFileMapping]:
        template = await self.get(db, template_id, organization_id)
    ...
    async def get_custom(self, db: AsyncSession, template_id: uuid.UUID, organization_id: uuid.UUID) -> CustomizeTemplate | None:
        stmt = (
            select(CustomizeTemplate)
            .join(TemplateVersion, CustomizeTemplate.template_version_id == TemplateVersion.template_version_id)
            .where(
                TemplateVersion.template_id == template_id,
                CustomizeTemplate.organization_id == organization_id,
    ...
    async def create_custom(self, db: AsyncSession, template_id: uuid.UUID, data: dict, created_by: uuid.UUID, organization_id: uuid.UUID) -> CustomizeTemplate:
        await self.get(db, template_id, organization_id)
        existing = await self.get_custom(db, template_id, organization_id)
        if existing:
            raise HTTPException(status_code=http_status.HTTP_409_CONFLICT, detail="Customize template already exists for this org")
    ...
    async def update_custom(self, db: AsyncSession, template_id: uuid.UUID, data: dict, organization_id: uuid.UUID) -> CustomizeTemplate:
        custom = await self.get_custom(db, template_id, organization_id)
        if not custom:
